...
void usage()
{
    cerr << "Usage: generate_phantoms [velocity_x] [velocity_y] [velocity_z] [N] [g_values] [g_directions] <options>\n" << endl;
    cerr << endl;
...
int main(int argc, char **argv)
{
    ...
    // read velocity volumes
    
    cout << "................................................................" << endl;
    cout << "................................................................" << endl;
    
    cout << " - Original velocity files : " << endl;
    ...
    for (i=0; i<3; i++) {
        ...
        stack = stack.GetRegion(100, 100, 200, 300, 300, 400);
        stack.PutOrigin(0, 0, 0);
        velocity_volumes.push_back(stack);

        argc--;
        argv++;
        
        sprintf(buffer,"original-velocity-%i.nii.gz", i);
        stack.Write(buffer);
            ...
            for (int z=0; z<stack.GetZ(); z++)
                for (int y=0; y<stack.GetY(); y++)
                    for (int x=0; x<stack.GetX(); x++)
                        stack(x,y,z,v) = velocity_volumes[v](x,y,z);

        }
        sprintf(buffer, "original-velocity-vector.nii.gz");
        stack.Write(buffer);
    ...
    cout<<" - Gradient values from " << g_val_file << " : "<<endl;
    if (in_g_val.is_open()) {
        while (!in_g_val.eof()) {
            in_g_val >> num;
            if (i<nStacks)
    ...
    int coord = 0;
    int dir = 0;
    cout<<" - Gradient directions from " << g_dir_file << " : "<<endl;
    if (in_g_dir.is_open()) {
        while (!in_g_dir.eof()) {
            in_g_dir >> num;
    ...
    // generating phase from velocity

    cout << endl;
    cout << "................................................................" << endl;
    cout << "GeneratePhaseVolumes : " << endl;
    
    stacks.clear();
    stacks = reconstruction.GeneratePhaseVolumes(velocity_volumes, g_directions, g_values);
    
    for (i=0; i<nStacks; i++) {
        
        sprintf(buffer,"generated-original-phase-%i.nii.gz", i);
    ...
    while (argc > 1){
        ...
        // Range for random slice translation magnitude
        if ((ok == false) && (strcmp(argv[1], "-t_range") == 0)){
            argc--;
            argv++;
            t_range=atoi(argv[1]);
            argc--;
        ...
        // Range for random slice rotation magnitude
        if ((ok == false) && (strcmp(argv[1], "-r_range") == 0)){
            argc--;
            argv++;
            r_range=atoi(argv[1]);
            argc--;
    ...
    cout << endl;
    cout << "................................................................" << endl;
    cout << "GenerateRandomPhaseVolumes : T range = " << t_range << " | R range = " << r_range <<endl;
    cout << endl;
    
    Array<int> stack_numbers;
    for (i=0; i<stacks.size(); i++)
        stack_numbers.push_back(i);
    
    bool all_slices = true;
    
    reconstruction.GenerateRandomPhaseVolumes(velocity_volumes, stacks, t_range, r_range, all_slices, stack_numbers);
...
//        reconstruction.EStepVelocity4D();
    ...
    // alpha
    // save updated gradient vectors
    // the impact of the case where one region is not covered by all stacks
    // impact of robust statistics
...
//        double consistency = reconstruction.Consistency();
        ...
        if (iteration > 30) { // (iteration+1)<rec_iterations ) { // && (iteration>(round(rec_iterations/2)))
